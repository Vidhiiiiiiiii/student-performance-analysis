...
# FIX #1: Corrected file paths for Linux (forward slash, correct path)
...
def clean_quotes(df):
    """Remove quote marks from all columns"""
...
def convert_data_types(df):
    ...
    # FIX #2: Changed 's' to 'failures' (typo fixed)
    ...
    # FIX #3: Fixed double loop - changed inner for loop to if statement
...
print(f"  Math dtypes sample: {dict(list(math_df.dtypes.items())[:5])}")
print(f"  Portuguese dtypes sample: {dict(list(portuguese_df.dtypes.items())[:5])}")
...
def validate_and_clean(df):
    ...
    if invalid_age == 0 and invalid_absences == 0:
        print("  ✓ No invalid values found - data is clean!")
    
    removed = initial_rows - len(df)
    # FIX #4: Added f-string prefix for proper string interpolation
    if removed > 0:
...
def engineer_features(df):
    ...
    df['struggling'] = (df['G3'] < 10).astype(int)
...
# FIX #5: Corrected file paths for saving
...
print("  /home/claude/math_cleaned_data.csv")
print("  /home/claude/portuguese_cleaned_data.csv")
